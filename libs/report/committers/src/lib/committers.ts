...
export const reportCommittersCommand: Command<Options> = {
  command: 'committers',
  group: 'report',
...
async function run(args: ArgumentsCamelCase<Options>): Promise<void> {
  const { startDate, endDate } = parseDateFlags(
    dateFormat,
    args.endDate,
    args.startDate
  );
  const ignores =
    args.exclude && args.exclude.length
      ? `-- . "!(${args.exclude.join('|')})"`
      : '';
...
function outputCommitters(committerCounts: CommitterCount[]) {
  ...
  console.log(
    committerCounts
      .map((c) => {
        const committer = `${c.name}${' '.repeat(
          longestNameLength - c.name.length
        )}`;
